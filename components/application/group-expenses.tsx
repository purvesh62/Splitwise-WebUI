...
export default function GroupExpensesTable({userGroup, group}: { userGroup: GroupExpenses[], group: SplitwiseGroup }) {
	...
	return <Table>
		...
		<TableHeader>
			<TableRow>
				<TableHead className="md:min-w-[500px] min-w-[50px]">Description</TableHead>
		...
		<TableBody>
			{userGroup.map((expense) => (
				<TableRow key={expense.id}>
					<TableCell className="font-medium md:min-w-[500px] w-[70px]">{expense.description}</TableCell>
					<TableCell>
						<div className={"flex md:flex-col"}>
							<Image className="rounded-full" src={expense.created_by?.picture?.medium as string}
							       alt={expense.created_by.first_name} width={30} height={30}/>
